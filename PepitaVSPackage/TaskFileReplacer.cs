...
public class TaskFileReplacer
{
    ...
    void DeployFiles(List<string> newStrings, string trimmed)
    {
        ...
        try
        {
            ...
            var info = string.Format("PepitaGet: Updated '{0}' to version {1}.", trimmed, CurrentVersion.Version);
        ...
        catch (Exception exception)
        {
            if (success)
            {
                var error = string.Format("PepitaGet: Failed to update '{0}' to version {1} due to '{2}'. Please manually copy the contents of '{3}' to '{0}'.", trimmed, CurrentVersion.Version, exception.Message, contentsFinder.ContentFilesPath);
                messageDisplayer.ShowError(error);
            }
            else
            {
                var info = string.Format("PepitaGet: Failed to update '{0}' to version {1} will try again later.", trimmed, CurrentVersion.Version);
                messageDisplayer.ShowInfo(info);
